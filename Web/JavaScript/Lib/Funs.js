 ...
 * as
 * @as.url	 {[string]}  		请求地址
 * @as.@method  {[string]} 		请求方式 
 * @as.arg  {[string]}			请求参数
 * @as.callback {[Function]}	请求成功后的回调函数
 */
function ajax( as ){
...
function getUnicode(str) {
	if(str.length > 1){
		throw Error("argument's length Error!");
	}
	// 1. 获取字符的字符编码
	// 2. 将获取到的字符编转换成 16 进制的字符串
	// 3. 最后用字面量加上 \u 前缀
	var before = str;
	var Code16 = before.charCodeAt(0).toString(16);
	// var charCode16 = charCode.toString(16);
	return "\\u"+ Code16;
}

// 将一个 Unicode 字符转换为一个 JS 可识别字符
function getCharFromUnicode(str){
	var js = '{"char":"'+str+'"}';
	// 利用 JSON 来解析 Unicode 字符
	return JSON.parse(js).char;
}
